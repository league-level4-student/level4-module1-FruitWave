...
public class _00_SnakeGame implements ActionListener, KeyListener {
	...
	public void startGame() {
		...
		String instructions = "DON'T DO WHAT YOU BELIEVE YOU SHOULD DO";
	...
	private void setFoodLocation() {
		...
		int ra = new Random().nextInt(15);
		int ar = new Random().nextInt(12);
		Location stuffurself = new Location(ra, ar);
		// 2. set the foodLocation variable equal to the Location object you just
		// created.
		while (snake.isLocationOnSnake(stuffurself)) {
			ra = new Random().nextInt(15);
			ar = new Random().nextInt(12);
			stuffurself = new Location(ra, ar);
		}
		foodLocation = stuffurself;
	...
	private void gameOver() {
		...
		JOptionPane.showMessageDialog(null, "u have become dead");
		// 3. ask them if they want to play again.
		String in = JOptionPane.showInputDialog("aye mate u wanna go anotha round?");
		boolean out = Boolean.parseBoolean(in);
		// 4. if they want to play again
		if (out) {
			int ra = new Random().nextInt(15);
			int ar = new Random().nextInt(12);
			Location stuffurself = new Location(ra, ar);
			snake.reset(stuffurself);
			setFoodLocation();
			timer.restart();
		} else {
			System.exit(0);
